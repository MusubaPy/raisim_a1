...
class ENVIRONMENT : public RaisimGymEnv {
 ...
 public:
  ...
  explicit ENVIRONMENT(const std::string& resourceDir, const Yaml::Node& cfg, bool visualizable) :
    ...
    gc_init_ << 0.1, 0.0, 0.37, 1.0, 0.0, 0.0, 0.0, 0.06, 0.6, -1.2, -0.06, 0.6, -1.2, 0.06, 0.6, -1.2, -0.06, 0.6, -1.2;
    ...
    jointPgain(0+6) = 40.0; jointPgain(1+6) = 40.0; jointPgain(2+6) = 40.0;
    jointPgain(3+6) = 40.0; jointPgain(4+6) = 40.0; jointPgain(5+6) = 40.0;
    jointPgain(6+6) = 40.0; jointPgain(7+6) = 40.0; jointPgain(8+6) = 40.0;
    jointPgain(9+6) = 40.0; jointPgain(10+6) = 40.0; jointPgain(11+6) = 40.0;

    jointDgain(0+6) = 1.0; jointDgain(1+6) = 1.0; jointDgain(2+6) = 1.0;
    jointDgain(3+6) = 1.0; jointDgain(4+6) = 1.0; jointDgain(5+6) = 1.0;
    jointDgain(6+6) = 1.0; jointDgain(7+6) = 1.0; jointDgain(8+6) = 1.0;
    jointDgain(9+6) = 1.0; jointDgain(10+6) = 1.0; jointDgain(11+6) = 1.0;
...
//    std::cout<<"Idx contact1 = "<<a1_->getBodyIdx("FL_calf")<<std::endl;
    ...
    if (visualizable_) 
    {
      ...
      a1_gui::init({//a1_gui::video::init(vis->getResourceDir()),
                    ...
                    //a1_gui::reward::init({"GenTorque", "GenVelocity", "Command"}),
                    a1_gui::reward::init({"BodyHeight", "GeneralBodyOrientation", "GeneralHipsAngles", "YaxisPos", "ClearAndSlip", "SmoothTorqVel", "Command", "GenVelocity", "GenTorque"}),
                    a1_gui::frame::init()});
  ...
  float step(const Eigen::Ref<EigenVec>& action) final {
    ...
    rewards_.record("Command", command);
    rewards_.record("SmoothTorqVel", SmoothTorqVel);
    rewards_.record("ClearAndSlip", ClearAndSlip);

    rewards_.record("YaxisPos", abs(gc_[1]));
    ...
    rewards_.record("AirTime", nCont);

    
    //rewards_.record("ForwardVel", std::min(3.0, bodyLinearVel_[0]));
    //rewards_.record("ForwardVel", BodyVel);
    //rewards_.record("ForwardPos", gc_[0]);
    ...
    //a1_gui::reward::log("ForwardPos", gc_[0] * rewards_.getCoef("ForwardPos"));
    a1_gui::reward::log("ForwardVel", BodyVel * rewards_.getCoef("ForwardVel"));
    //a1_gui::reward::log("ForwardVel", BodyVel);
    a1_gui::reward::log("ClearAndSlip",  ClearAndSlip * rewards_.getCoef("ClearAndSlip"));
    a1_gui::reward::log("BodyHeight",  BodyHeight * rewards_.getCoef("BodyHeight"));
    a1_gui::reward::log("SmoothTorqVel",  SmoothTorqVel * rewards_.getCoef("SmoothTorqVel"));
    a1_gui::reward::log("GeneralHipsAngles",  generalHipsAngles * rewards_.getCoef("GeneralHipsAngles"));
//generalBodyOrientation * rewards_.getCoef("GeneralBodyOrientation")
    */
    a1_gui::reward::log("GenTorque", a1_->getGeneralizedForce().squaredNorm() * rewards_.getCoef("GenTorque"));
    a1_gui::reward::log("GenVelocity",a1_->getGeneralizedVelocity().squaredNorm() * rewards_.getCoef("GenVelocity"));
    a1_gui::reward::log("Command", command * rewards_.getCoef("Command"));
    a1_gui::reward::log("SmoothTorqVel",  SmoothTorqVel * rewards_.getCoef("SmoothTorqVel"));
    a1_gui::reward::log("ClearAndSlip",  ClearAndSlip * rewards_.getCoef("ClearAndSlip"));

    a1_gui::reward::log("YaxisPos", abs(gc_[1]) * rewards_.getCoef("YaxisPos"));

    a1_gui::reward::log("GeneralHipsAngles", generalHipsAngles * rewards_.getCoef("GeneralHipsAngles"));
    a1_gui::reward::log("GeneralBodyOrientation", generalBodyOrientation * rewards_.getCoef("GeneralBodyOrientation"));
    a1_gui::reward::log("BodyHeight", BodyHeight * rewards_.getCoef("BodyHeight"));

    a1_gui::reward::log("AirTime", nCont * rewards_.getCoef("AirTime"));
...
double calcFootClearanceAndSlipCost(double& footClearanceCost, double& footSlipCost) {
  ...
  void updateObservation() {
    ...
    //airRotation_[2] = abs(airRotation_[2]) - 1;
    //std::cout<<"air Rotation ="<<airRotation_<<std::endl;
    generalHipsAngles = abs(gc_init_[7]-gc_[7]) + abs(gc_init_[10]-gc_[10]) + abs(gc_init_[13]-gc_[13]) + abs(gc_init_[16]-gc_[16]);
    //generalBodyOrientation = abs(bodyAngularVel_[0]) + abs(bodyAngularVel_[1]) + abs(bodyAngularVel_[2]);
    generalBodyOrientation = abs(airRotation_[0]) + abs(airRotation_[1]) + abs(airRotation_[2]);
    //generalBodyOrientation = abs(bodyAngularVel_[0]) + abs(bodyAngularVel_[1]) + abs(bodyAngularVel_[2]);

    
    command = wishVel - abs(wishVel - bodyLinearVel_[0]); 
    ...
    generalHipsAngles = 1 - generalHipsAngles;
    ...
    rewardSmoothVel = 0.0;
    rewardSmoothTorq = 0.0;
    for (int i = 0; i<12; i++)
    {
      //rewardSmoothVel = rewardSmoothVel + std::max(abs(obVelocity_(i) - jointSpeed(i)), 1.00);
      //rewardSmoothTorq = rewardSmoothTorq + std::max(abs(obTorque_(i) - jointTorque(i)), 1.00);
      rewardSmoothVel = rewardSmoothVel + abs(obVelocity_(i) - jointSpeed(i));
      rewardSmoothTorq = rewardSmoothTorq + abs(obTorque_(i) - jointTorque(i));
    }
    //SmoothTorqVel = std::min(360.00, round(rewardSmoothVel*10e5)/10e5) + std::min(1920.00, round(rewardSmoothTorq*10e5)/10e5);
    //SmoothTorqVel = 1200 - std::min(1200.00, round(rewardSmoothVel*10e5)/10e5);
    SmoothTorqVel = 240 - std::min(240.00, rewardSmoothVel) + 2260 - std::min(2260.00, rewardSmoothTorq);
    //std::cout<<"obVelocity_ ="<<SmoothTorqVel<<std::endl;
    //SmoothTorqVel = 1;
...
//    normalno = contact.normal().e().transpose();
 ...
 private:
  ...
  Eigen::VectorXd gc_init_, gv_init_, gc_, gv_, pTarget_, pTarget12_, vTarget_, normalForce, sumNormalForce;
  double terminalRewardCoeff_ = -5.00, netForce_ = 0.00, generalHipsAngles = 0.00, desired_fps_ = 50.00, generalBodyOrientation = 0.00;
  ...
  double footClearanceCost_ = 0.00, footSlipCost_ = 0.00, ClearAndSlip = 0.00, BodyHeight = 0.00, BodyVel = 0.00;
  double SmoothTorqVel, rewardSmoothVel, rewardSmoothTorq, command, wishVel, nCont;
